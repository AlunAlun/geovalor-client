...
function createWmsLayer(layerName) {
  return new window.google.maps.ImageMapType({
    getTileUrl: function (coord, zoom) {
      ...
      return (
        `https://wmts.mapama.gob.es/sig/costas/${layerName}/ows?` +
        `SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&LAYERS=${layerName}` +
...
function GoogleMapWithOverlay({ lat, lng }) {
  ...
  useEffect(() => {
    loadGoogleMapsScript(API_KEY).then(() => {
      ...
      const overlay100 = createWmsLayer("zim_laminas_q100");
      const overlay500 = createWmsLayer("zim_laminas_q500");
