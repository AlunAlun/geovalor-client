...
function createWmsLayer({ baseUrl, layerName }) {
  return new window.google.maps.ImageMapType({
    getTileUrl: function (coord, zoom) {
      ...
      const url =
        `${baseUrl}?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&LAYERS=${layerName}` +
...
function GoogleMapWithOverlay({ lat, lng, showFluvial, showCoastal }) {
  ...
  useEffect(() => {
    ...
    loadGoogleMapsScript(API_KEY).then(() => {
      ...
      // --- Marker at the passed coordinates ---
      ...
      // Keep map centered on the passed point
      ...
      if (showCoastal) {
        ...
        overlayIds.push(overlay100, overlay500);
      }
      ...
      if (showFluvial) {
        ...
        fluvialLayers.forEach((layerName, index) => {
          ...
          overlayIds.push(fluvialOverlay);
        });
      }
      ...
      if (showCoastal) {
        ...
        // Clean up legend on rerender
        return () => {
          const controls =
            map.controls[window.google.maps.ControlPosition.RIGHT_BOTTOM];
          for (let i = controls.getLength() - 1; i >= 0; i--) {
            const ctrl = controls.getAt(i);
            if (ctrl === legendDiv) controls.removeAt(i);
          }
        };
      }
    });

    // Cleanup on unmount: nothing heavy to remove since overlays/marker are tied to map instance.
    return () => {
      // If you ever recreate the map instance, you could call:
